...
describe('RTFParser', function () {
  ...
  describe('tokenization', function () {
    it('should allow uppercase and lowercase control words', co(function* () {
      ...
      expect(result[0]).to.eql({type: 'WORD', value: 'word'});
      expect(result[1]).to.eql({type: 'WORD', value: 'WoRd'});
    }));
    ...
    it('should eat the optional space after control words', co(function* () {
      ...
      expect(result[0]).to.eql({type: 'WORD', value: 'word'});
      expect(result[1]).to.eql({type: 'WORD', value: 'WoRd'});
      expect(result[2]).to.eql({type: 'TEXT', value: ' '});
    }));

    it('should allow control word numerical param', co(function* () {
      const result = yield process(['\\word001\\WoRd123']);
      expect(result).to.be.an('array').of.length(2);
      expect(result[0]).to.eql({type: 'WORD', value: 'word', param: 1});
      expect(result[1]).to.eql({type: 'WORD', value: 'WoRd', param: 123});
    }));
    ...
    it('should return basic control symbol escapes as chars', co(function* () {
      const result = yield process(['\\word0 hi\\', '{\\\\\\', '}']);
      expect(result).to.be.an('array').of.length(5);
      expect(result[0]).to.eql({type: 'WORD', value: 'word', param: 0});
      expect(result[1]).to.eql({type: 'TEXT', value: 'hi'});
      expect(result[2]).to.eql({type: 'TEXT', value: '{'});
      expect(result[3]).to.eql({type: 'TEXT', value: '\\'});
      expect(result[4]).to.eql({type: 'TEXT', value: '}'});
    }));

    it('should return escaped control symbol \\n, \\r, \\t as text', co(function* () {
      const result = yield process(['\\word0 hi\\', '\r\\\n\\\t']);
      expect(result).to.be.an('array').of.length(5);
      expect(result[0]).to.eql({type: 'WORD', value: 'word', param: 0});
      expect(result[1]).to.eql({type: 'TEXT', value: 'hi'});
      expect(result[2]).to.eql({type: 'TEXT', value: '\r'});
      expect(result[3]).to.eql({type: 'TEXT', value: '\n'});
      expect(result[4]).to.eql({type: 'TEXT', value: '\t'});
    }));

    it('should properly skip binary data', co(function* () {
      const result = yield process(['\\bin', '4 \\hi2\\hi3']);
      expect(result).to.be.an('array').of.length(2);

      const buf = new Buffer('\\hi2');
      expect(result[0]).to.eql({type: 'WORD', value: 'bin', param: 4, data: buf, length: 4});
      expect(result[1]).to.eql({type: 'WORD', value: 'hi', param: 3});
    }));
    ...
    it('should handle \\bin control word without positive param', co(function* () {
      ...
      expect(result[0]).to.eql({type: 'WORD', value: 'bin'});
      expect(result[1]).to.eql({type: 'WORD', value: 'bin', param: 0});
      expect(result[2]).to.eql({type: 'WORD', value: 'bin', param: -10});
    }));
    ...
    it('should handle \\bin control with early EOF', co(function* () {
      ...
      expect(result[0]).to.have.property('value', 'bin');
